...
import { createProduct, getSellerProducts, updateProduct, deleteProduct, getAllProducts, getProductById } from '../controllers/productController';

const router = Router();
router.get('/public', getAllProducts); // buyer browse
router.get('/public/:id', getProductById); // get single product
router.use(protect);
